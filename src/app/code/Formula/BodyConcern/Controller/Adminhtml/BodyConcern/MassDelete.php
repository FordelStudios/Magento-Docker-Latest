    ...
    public function execute()
    {
        ...
        try {
            ...
            if (!$ids) {
                ...
                if ($ids === null) {
                    $this->messageManager->addErrorMessage(__('Please select bodyconcern(s) to delete.'));
                ...
                if ($excluded && $excluded[0] !== '') {
                    $collection->addFieldToFilter('bodyconcern_id', ['nin' => $excluded]);
                }
            ...
            foreach ($ids as $id) {
                try {
                    ...
                    $this->messageManager->addErrorMessage(
                        __('Error deleting bodyconcern ID %1: %2', $id, $e->getMessage())
                    );
                }
            }
            ...
            if ($deletedCount > 0) {
                $this->messageManager->addSuccessMessage(
                    __('A total of %1 bodyconcern(s) have been deleted.', $deletedCount)
                );
            }
        } catch (\Exception $e) {
